...
def _system_prompt(ctx: CopilotContext) -> str:
    ...
    return (
        ...
        "- For questions about RECORDED history (units sold, prices, revenue, promotions/events "
        "by item, store, department, state or date), use query_data to run read-only SQL over "
        "the sales data. That data has NO inventory positions or stockouts - those are simulated, "
        "so route stock/reorder/stockout questions to the simulation and item tools instead.\n\n"
